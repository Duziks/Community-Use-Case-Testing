...
if True:
    ...
    class CudaGraphTreeTests(TestCase):
        ...
        def test_mutation_on_inp(self, backend):
            ...
            # mutation on inp doesn't hit npugraphs
        ...
        def test_index_put_boolean_fallback_partitions_cudagraphs(self):
            ...
            if config.graph_partition:
                ...
                with capture_stderr() as captured_output:
                    fn_c(x, mask, values)
                FileCheck().check("skipping npugraphs due to").run(captured_output[0])
        ...
        def test_workspace_allocation_error(self):
            ...
            try:
                ...
                try:
                    ...
                    if not IS_ARM64:
                        self.assertTrue(
                            "at::npu::blas::gemm<float, float>" in str(e)
                            or "at::npu::blas::gemm_internal_cublas<float, float>"
                            in str(e)
                        )
                        self.assertTrue(
        ...
        def test_incompatible_cudagraph_ops_item(self):
            ...
            if config.graph_partition:
                ...
                FileCheck().check(
                    "skipping npugraphs due to disabling cudagraphs due to incompatible op"
                ).run(captured_output[0])
